...
class HeatmapOffsetParser:
    ...
    def refine_score(self, heatmaps, poses):
        ...
        """
        normed_poses = poses.unsqueeze(0).permute(2, 0, 1, 3).contiguous()  # [NxKx4]→[1xNxKx4]→[Kx1xNx4]
        normed_poses = torch.cat((
            normed_poses.narrow(3, 0, 1) / (heatmaps.size(3) - 1) * 2 - 1,  # tensor.narrow(dim, start, length),张量索引切片
            normed_poses.narrow(3, 1, 1) / (heatmaps.size(2) - 1) * 2 - 1,
        ),
                                 dim=3)                                     # pose转成[-1,1]之间的数
...
class HeatmapOffsetParserInclusiveRLE:
    """The heatmap&offset parser for post processing."""

    def __init__(self, cfg):
        super(HeatmapOffsetParserInclusiveRLE, self).__init__()

        self.num_joints = cfg['num_joints']
        self.keypoint_threshold = cfg['keypoint_threshold']
        self.max_num_people = cfg['max_num_people']
        self.use_sigma_as_score = cfg['use_sigma_as_score']
    ...
    def _offset_to_pose(self, offsets):
        ...
        Returns:
            torch.Tensor[NxKxHxW]: A tensor containing pose for each pixel.
            torch.Tensor[NxKxHxW]: A tensor containing all pose scores for each pixel.
        """
        h, w = offsets.shape[-2:]
        offsets = offsets.reshape(self.num_joints, -1, h, w)   #offsets[N,K*D,h,w]→[K,D,h,w]
        ...
        posemaps = regular_coords.to(offsets) - offsets[:,:2,:,:]   # offsets[K,D,h,w],不使用RLELoss，D=2, 使用RLELoss, D=4(μ_y,μ_x,σ_y,σ_x)
        posemaps = posemaps.reshape(1, -1, h, w)
        
        sigmamap = offsets[:,2:,:,:]
        sigmamap = sigmamap.reshape(1, -1, h, w)
        
        return posemaps, sigmamap
    ...
    def decode(self, heatmaps, offsets):
        ...
        posemap, sigmamap = self._offset_to_pose(offsets) # [1, k*2, h, w]
        inst_indexes, inst_scores = self._get_maximum_from_heatmap(heatmaps[:, :1])    #1维tensor

        poses = posemap.reshape(posemap.size(1), -1)[..., inst_indexes]    #[k*2, m], m为候选实例数
        poses = poses.reshape(self.num_joints, 2, -1).permute(2, 0, 1).contiguous()    # [m,k,2]
        
        inst_scores = inst_scores.unsqueeze(1).unsqueeze(2).expand(poses.size())
        poses = torch.cat((poses, inst_scores), dim=2)
        
        # 将offset第3、4维的σ作为关键点置信度，与inst_scores融合
        if self.use_sigma_as_score:
            # offset的sigma
            sigma = sigmamap.reshape(sigmamap.size(1), -1)[..., inst_indexes] #[k*2, m]
            sigma = sigma.reshape(self.num_joints, 2, -1).permute(2, 0, 1).contiguous()  # [m,k,2]
            sigma = (torch.ones_like(sigma)-torch.sigmoid(sigma)).mean(-1)
            poses[:,:,3] = sigma    # 关键点置信度
            poses[:,:,2] = poses[:,:,2]*sigma   # 关键点置信度与实例分数相结合
        return poses.clone()
    ...
    def refine_score(self, heatmaps, poses):
        ...
        Returns:
            torch.Tensor[NxKx4]: poses with refined scores.
        """
        assert heatmaps.size(1)>1, 'no keypoint heatmaps'
        normed_poses = poses.unsqueeze(0).permute(2, 0, 1, 3).contiguous()  # [N,k,4]→[1,N,k,4]→[k,1,N,4]
        normed_poses = torch.cat(
            (
                normed_poses.narrow(3, 0, 1) / (heatmaps.size(3) - 1) * 2 - 1,  # tensor.narrow(dim, start, length),张量索引切片
                normed_poses.narrow(3, 1, 1) / (heatmaps.size(2) - 1) * 2 - 1,
        ...
        kpt_scores = torch.nn.functional.grid_sample(
            heatmaps[:, 1:].reshape(self.num_joints, 1, heatmaps.size(2), heatmaps.size(3)),
            normed_poses,
            padding_mode='border'
