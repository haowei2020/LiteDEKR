...
from mmpose.core.post_processing.group import HeatmapOffsetParser, HeatmapOffsetParserInclusiveRLE
...
# from mmpose.models.utils.rescore import DekrRescoreNet
...
@POSENETS.register_module()
class DisentangledKeypointRegressorWithRLE(BasePose):
    """在DisentangledKeypointRegressor基础上修改, 
        1、self.parser = HeatmapOffsetParserInclusiveRLE(self.test_cfg).
           匹配keypoint_head[type]=DEKRHeadWithRLELoss;
           可选网络的输出的σ做关键点置信度；
        2、refine_keypoint_scores_by_keypoint_heatmaps可选;
        3、关键点热力图还是需要的
    ...
    def __init__(self,
        ...
        self.test_cfg = test_cfg
        self.use_udp = test_cfg.get('use_udp', False)
        self.parser = HeatmapOffsetParserInclusiveRLE(self.test_cfg)
        self.refine_keypoint_scores_by_keypoint_heatmaps = \
            test_cfg['refine_keypoint_scores_by_keypoint_heatmaps']
        self.pretrained = pretrained
    ...
    def forward_test(self, img, img_metas, return_heatmap=False, **kwargs):
        ...
        for idx, s in enumerate(sorted(test_scale_factor, reverse=True)):
            ...
            if self.test_cfg.get('flip_test', True):
                ...
                # compute offsets for flipped input image
                h, w = offsets_flipped.shape[2], offsets_flipped.shape[3]
                offsets_flipped = offsets_flipped.view(num_joints, 4, h, w)   
                offsets_flipped = offsets_flipped.transpose(1, 0).contiguous()               
                offsets_flipped[0] = -offsets_flipped[0] - 1
                offsets_flipped = flip_feature_maps([offsets_flipped],
                                                    flip_index=flip_index)[0]
        ...
        poses = torch.cat(tuple(scale_poses_dict.values()))
        # refine keypoint scores using keypoint heatmaps
        if self.refine_keypoint_scores_by_keypoint_heatmaps:
            poses = self.parser.refine_score(aggregated_heatmaps, poses)
        poses = poses.cpu().numpy()
...
@POSENETS.register_module()
class DisentangledKeypointRegressorWithRLE_V2(BasePose):
    """在DisentangledKeypointRegressor基础上修改, 
        1、offset的通道数为k*4, test时, 使用前两维作为回归结果；
        2、使用更小的特征图(16倍下采样)估计热力图和offset, 通过data_cfg['heatmap_size']、keypoint_head['in_index']调整，其他超参数略作调整
        3、不估计关键点的热力图, 只估计中心点的热力图, 通过keypoint_head['use_keypoint_heatmaps']设置
        4、NMS时排序使用的置信度由offset部分预测的σ提供, σ在推理时经sigmoid处理, 由test_cfg['use_sigma_as_score']控制
        5、删除refine keypoint scores using keypoint heatmaps操作, 关键点热力图可以去掉
    Args:
        backbone (dict): Backbone modules to extract feature.
        keypoint_head (dict): Keypoint head to process feature.
        train_cfg (dict): Config for training. Default: None.
    ...
    def forward_test(self, img, img_metas, return_heatmap=False, **kwargs):
        ...
        for idx, s in enumerate(sorted(test_scale_factor, reverse=True)):
            ...
            if self.test_cfg.get('flip_test', True):
                ...
                # compute offsets for flipped input image
                h, w = offsets_flipped.shape[2], offsets_flipped.shape[3]
                offsets_flipped = offsets_flipped.view(num_joints, 4, h, w)   # 2改为4
                offsets_flipped = offsets_flipped.transpose(1, 0).contiguous()               
                offsets_flipped[0] = -offsets_flipped[0] - 1
                offsets_flipped = flip_feature_maps([offsets_flipped],
                                                    flip_index=flip_index)[0]
            ...
            poses = self.parser.decode(aggregated_heatmaps, aggregated_offsets) # use_sigma_as_score, (1-sigmoid(offsets[:,2:3])).mean(-1)作为关键点的置信度，即poses[:,3];poses[:,2]为实例的分数和关键点分数的乘积
            # rescale pose coordinates to a unified scale
            poses[..., :2] *= (image_size * 1.0 / heatmap_size) / s
            scale_poses_dict[s] = poses
